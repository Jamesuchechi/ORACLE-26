...
const SimulatorView = () => {
  ...
  if (loading) return <div className="p-12 text-center font-mono text-white/20 animate-pulse uppercase tracking-widest text-xs">Simulating 1,000 Tournament Permutations...</div>;
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex gap-4 border-b border-white/5 pb-4">
        <button 
          ...
          className={`px-4 py-2 rounded-lg text-[10px] font-mono uppercase tracking-widest transition-all ${activeTab === 'groups' ? 'bg-amber/10 text-amber border border-amber/20' : 'text-white/20 hover:text-white/40'}`}
        >
          Group Standings
        </button>
        <button 
          onClick={() => setActiveTab('probabilities')}
          className={`px-4 py-2 rounded-lg text-[10px] font-mono uppercase tracking-widest transition-all ${activeTab === 'probabilities' ? 'bg-amber/10 text-amber border border-amber/20' : 'text-white/20 hover:text-white/40'}`}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {groups.map(([name, teams], idx) => (
            <motion.div
              ...
              className="terminal-card bg-bg1/20 border-white/5 overflow-hidden"
            >
              <div className="bg-white/5 px-4 py-3 flex justify-between items-center border-b border-white/5">
                <h3 className="font-mono text-[10px] font-bold text-white/60 tracking-widest uppercase">Group {name}</h3>
                <Trophy size={12} className="text-white/10" />
              </div>
              <div className="p-4 space-y-4">
                {teams.map((t, i) => (
                  <div key={t.team} className="flex items-center gap-3 relative group">
                    <div className="w-4 text-[9px] font-mono text-white/20">{i + 1}</div>
                    <img 
                    ...
                    <div className="flex-1">
                      <p className={`text-xs font-medium ${i < 2 ? 'text-white/90' : 'text-white/40'}`}>{t.team}</p>
                    </div>
              ...
              <div className="space-y-6">
                {data.advancement.slice(0, 15).map((t, idx) => (
                  <div key={t.team} className="space-y-2">
                    ...
                    <div className="flex h-1.5 gap-0.5 rounded-full overflow-hidden bg-white/5">
            ...
            <div className="terminal-card bg-amber/5 border-amber/20 p-6">
              ...
              <div className="space-y-6">
                <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
                  <p className="text-[10px] font-mono text-white/20 uppercase mb-2">Likely Finalists</p>
                  <div className="flex items-center justify-between text-xs font-bold">
                    <div className="flex items-center gap-2">
                      <img src={getFlagUrl(data.advancement[0].team)} className="w-4 h-3 rounded-sm" />
                      {data.advancement[0].team}
                    </div>
                    <div className="text-white/20 font-light">VS</div>
                    <div className="flex items-center gap-2">
                      {data.advancement[1].team}
                      <img src={getFlagUrl(data.advancement[1].team)} className="w-4 h-3 rounded-sm" />
                    </div>
                  </div>
                  <div className="mt-4 pt-4 border-t border-white/5 text-[10px] italic text-white/40 leading-relaxed">
